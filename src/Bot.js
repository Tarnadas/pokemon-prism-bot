  ...
  async onMessage(message) {
    ...
    if (this.waitForOption[clientId]) {
      try {
        ...
        if (
          this.option[clientId] < 0 ||
          this.option[clientId] >= this.optionSize[clientId]
        ) {
    ...
    for (let i in attachments) {
      ...
      console.log(
        `New file uploaded by user ${message.author.username} with MD5: ${md5(
          fileBuf
        )}`
      );
    }
    ...
    if (!this.bsp || !(this.bsp instanceof ArrayBuffer)) {
      message.author.send(
        "I couldn't find a valid patch file. Please try again later."
      );
      return;
    }
    ...
    try {
      ...
      const { code, options, data, messages } = await this.patcher[
        clientId
      ].patch(this.option[clientId]);
